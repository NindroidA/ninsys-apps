...
export function useOnboardingConfig(guildId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<OnboardingConfig>(
        `/guilds/${guildId}/onboarding/config`
      );
...
export function useOnboardingSteps(guildId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<OnboardingStep[]>(
        `/guilds/${guildId}/onboarding/steps`
      );
...
export function useCreateOnboardingStep(guildId: string) {
  ...
  return useMutation({
    mutationFn: async (data: Omit<OnboardingStep, "id" | "sortOrder">) => {
      const result = await apiPost<OnboardingStep>(
        `/guilds/${guildId}/onboarding/steps/add`,
        data
      );
  ...
  return useMutation({
    mutationFn: async (stepId: string) => {
      const result = await apiPost(
        `/guilds/${guildId}/onboarding/steps/remove`,
        { stepId }
      );
...
export function useReorderOnboardingSteps(guildId: string) {
  ...
  return useMutation({
    mutationFn: async (stepIds: string[]) => {
      const result = await apiPost(
        `/guilds/${guildId}/onboarding/steps/reorder`,
        { stepIds }
      );
...
export function useOnboardingStats(guildId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<OnboardingStats>(
        `/guilds/${guildId}/onboarding/stats`
      );
      if (!result.success || !result.data) return null;
      return result.data;
    },
