...
export function useAnnouncementConfig(guildId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<AnnouncementConfig>(
  ...
  return useMutation({
    mutationFn: async (payload: AnnouncementPayload) => {
      const result = await apiPost(
        `/guilds/${guildId}/announcements/send`,
        payload
      );
...
export function useAnnouncementHistory(
  guildId: string,
  params: AnnouncementHistoryParams = {}
) {
...
export function useDeleteAnnouncementTemplate(guildId: string) {
  ...
  return useMutation({
    mutationFn: async (templateName: string) => {
      const result = await apiPost(
        `/guilds/${guildId}/announcements/templates/delete`,
        { name: templateName }
      );
