...
export function useEventsConfig(guildId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<EventsConfig>(
        `/guilds/${guildId}/events/config`
      );
...
export function useUpdateEventsConfig(guildId: string) {
  ...
  return useMutation({
    mutationFn: async (data: Partial<EventsConfig>) => {
      const result = await apiPost<EventsConfig>(
        `/guilds/${guildId}/events/config/update`,
        data
      );
...
export function useEventTemplates(guildId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<EventTemplate[]>(
  ...
  return useMutation({
    mutationFn: async (templateId: string) => {
      const result = await apiPost(
        `/guilds/${guildId}/events/templates/delete`,
        { templateId }
      );
...
export function useUpcomingReminders(guildId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<UpcomingReminder[]>(
        `/guilds/${guildId}/events/reminders`
      );
      if (!result.success || !result.data) return [];
      return Array.isArray(result.data) ? result.data : [];
    },
