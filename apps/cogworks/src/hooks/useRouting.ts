...
export function useRoutingConfig(guildId: string) {
	return useQuery({
		...
		queryFn: async () => {
			const result = await apiGet<RoutingConfig>(`/guilds/${guildId}/tickets/routing/config`);
...
export function useUpdateRoutingConfig(guildId: string) {
	...
	return useMutation({
		mutationFn: async (data: Partial<RoutingConfig>) => {
			const result = await apiPut<RoutingConfig>(`/guilds/${guildId}/tickets/routing/config`, data);
...
export function useAddRoutingRule(guildId: string) {
	...
	return useMutation({
		mutationFn: async (data: {
			...
			const result = await apiPost(`/guilds/${guildId}/tickets/routing/rules`, data);
...
export function useDeleteRoutingRule(guildId: string) {
	...
	return useMutation({
		mutationFn: async (ruleId: string) => {
			const result = await apiDelete(`/guilds/${guildId}/tickets/routing/rules/${ruleId}`);
...
export function useRoutingStats(guildId: string) {
	return useQuery({
		...
		queryFn: async () => {
			const result = await apiGet<RoutingStats>(`/guilds/${guildId}/tickets/routing/stats`);
			if (!result.success || !result.data) return null;
			return result.data;
		},
		staleTime: 1000 * 60 * 2,
