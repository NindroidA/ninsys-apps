...
export function useSlaConfig(guildId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<SlaConfig>(
        `/guilds/${guildId}/tickets/sla/config`
      );
...
export function useUpdateSlaConfig(guildId: string) {
  ...
  return useMutation({
    mutationFn: async (data: Partial<SlaConfig>) => {
      const result = await apiPut<SlaConfig>(
        `/guilds/${guildId}/tickets/sla/config`,
        data
      );
...
export function useSlaStats(guildId: string, days = 30) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<SlaStats>(
        `/guilds/${guildId}/tickets/sla/stats?days=${days}`
      );
      if (!result.success || !result.data) return null;
      return result.data;
    },
