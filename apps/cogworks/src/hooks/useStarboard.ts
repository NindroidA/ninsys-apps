...
export function useStarboardConfig(guildId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<StarboardConfig>(
...
export function useStarboardStats(guildId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await apiGet<StarboardStats>(
        `/guilds/${guildId}/starboard/stats`
      );
      if (!result.success || !result.data) return null;
      return result.data;
    },
