...
import { useAddRole, useDeleteRole, useSavedRoles, useUpdateRole } from "@/hooks/useRoles";
	...
	return (
		<div className="px-4 py-3 border-b border-border last:border-b-0 space-y-2">
			<div className="flex items-center gap-3">
				...
				/>
				<span className="font-medium text-sm flex-1 min-w-0 truncate">{roleName}</span>
				<button
					type="button"
			...
			<div className="ml-6">
				<p className="text-[10px] uppercase tracking-wider text-muted-foreground/50 mb-1">Alias</p>
				{isEditing ? (
					<Input
						value={alias}
					...
					>
						{role.alias ? (
							<span className="text-sm text-accent flex-1 truncate">{role.alias}</span>
						) : (
							<span className="text-sm text-muted-foreground/50 italic flex-1">No alias set</span>
						)}
	...
	return (
		<div className="py-3 space-y-3">
			<RolePicker
				guildId={guildId}
				value={form.roleId}
				onChange={(roleId) => setForm((prev) => ({ ...prev, roleId: roleId as string | null }))}
			...
			<div className="flex items-center gap-2 justify-end">
				...
				<Button onClick={handleSave} disabled={!form.roleId || addRole.isPending}>
	...
	return (
		<ConfigSection title={title} description={description}>
			{filtered.length === 0 && !isAdding ? (
				<p className="text-sm text-muted-foreground text-center py-4">No {type} roles configured</p>
			) : (
			...
			{isAdding && <AddRoleRow type={type} guildId={guildId} onCancel={() => setIsAdding(false)} />}
