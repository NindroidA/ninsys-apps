...
import { useSlaConfig, useSlaStats, useUpdateSlaConfig } from "@/hooks/useSla";
			...
			<div className="space-y-6">
				{!configLoading && !config && (
					<div className="text-center py-12 border border-dashed border-border rounded-lg">
						<p className="text-muted-foreground">SLA tracking is not configured yet.</p>
				...
				{config && (
					<>
						...
						<ConfigSection title="SLA Configuration" description="Set target response times">
							<div>
								...
								<input
									...
									onChange={(e) =>
										updateConfig.mutate({
											targetResponseMinutes: Number.parseInt(e.target.value, 10) || 60,
										})
									}
							...
							<ChannelPicker
								...
								onChange={(channelId) => updateConfig.mutate({ breachAlertChannelId: channelId })}
								filter="text"
						...
						>
							{config.perTypeOverrides.length === 0 ? (
								<p className="text-sm text-muted-foreground">
									No per-type overrides configured. All ticket types use the default target of{" "}
									{config.targetResponseMinutes} minutes.
								</p>
							) : (
								<div className="space-y-2">
									{config.perTypeOverrides.map((override) => (
										...
										>
											<span className="text-sm font-medium">{override.typeName}</span>
						...
						<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
							...
							<Card className="p-4">
								<div className="flex items-center gap-3">
									...
									<div>
										<p className="text-2xl font-bold">{Math.round(stats.complianceRate)}%</p>
