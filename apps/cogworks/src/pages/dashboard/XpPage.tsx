...
import { Award, BarChart3, Download, Gift, Plus, Settings, Trash2, Trophy } from "lucide-react";
...
function XpConfigTab({ guildId }: { guildId: string }) {
	...
	return (
		<div className="space-y-6">
			...
			<ConfigSection title="Voice XP" description="XP earned from voice channels">
				<StatusToggle
					enabled={config.voiceXpEnabled}
					onChange={(enabled) => updateConfig.mutate({ voiceXpEnabled: enabled })}
					label="Voice XP"
			...
			<ConfigSection title="Level-Up Notifications">
				<ChannelPicker
					...
					onChange={(channelId) => updateConfig.mutate({ levelUpChannelId: channelId })}
					filter="text"
				...
				/>
				<div>
					<label className="text-sm font-medium block mb-1.5">Level-Up Message Template</label>
					<textarea
						value={config.levelUpMessage ?? ""}
						onChange={(e) => updateConfig.mutate({ levelUpMessage: e.target.value || null })}
			...
			<ConfigSection title="Multipliers">
				<StatusToggle
					...
					onChange={(enabled) => updateConfig.mutate({ stackMultipliers: enabled })}
				...
				<div>
					...
					{multipliers.length === 0 ? (
						<p className="text-sm text-muted-foreground">No channel multipliers configured.</p>
					) : (
...
function XpImportTab({ guildId }: { guildId: string }) {
	...
	const columns: Column<XpImportResult>[] = [
		{ key: "source", header: "Source" },
		{
			...
			render: (r) => (
				<Badge
					variant={
						r.status === "completed" ? "default" : r.status === "running" ? "secondary" : "error"
					}
				>
					{r.status}
				</Badge>
	...
	return (
		<div className="space-y-6">
			<ConfigSection title="Import from MEE6" description="Import existing XP data from MEE6">
				<Button onClick={() => importMee6.mutate({})} disabled={importMee6.isPending}>
