		...
		<section className="mb-8">
			...
			{isLoading ? (
				...
				<Card className="p-6 text-center">
					...
					<p className="text-sm text-muted-foreground">No recent activity from the dashboard yet</p>
				</Card>
			) : (
				<div className="space-y-2">
					{activity.map((entry) => (
						...
						>
							...
							<div className="flex-1 min-w-0">
								<p className="text-sm font-medium truncate">{formatAction(entry.action)}</p>
			...
			<section className="mb-8">
				...
				{isLoading ? (
					...
					<StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
						<StatCard label="Active Tickets" value={sys?.tickets?.activeCount ?? 0} icon={Ticket} />
						<StatCard
							label="Pending Applications"
							value={sys?.applications?.activeCount ?? 0}
							icon={FileText}
						/>
						<StatCard label="Memory Items" value={sys?.memory?.itemCount ?? 0} icon={Brain} />
						<StatCard
											...
											<StatusBadge
												...
												state={
													setupState?.systemStates?.[
														SYSTEM_STATE_MAP[system.key] as keyof typeof setupState.systemStates
													]
												}
											/>
