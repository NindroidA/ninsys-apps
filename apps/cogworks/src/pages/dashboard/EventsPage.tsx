		...
		<div className="space-y-6">
			...
			<ConfigSection title="Reminder Settings">
				<ChannelPicker
					...
					onChange={(channelId) => updateConfig.mutate({ reminderChannelId: channelId })}
					filter="text"
				...
				<div>
					...
					<input
						...
						onChange={(e) =>
							updateConfig.mutate({
								defaultReminderMinutes: Number.parseInt(e.target.value, 10) || 15,
							})
						}
			...
			<ConfigSection title="Post-Event Summary">
				<StatusToggle
					...
					onChange={(postEventSummary) => updateConfig.mutate({ postEventSummary })}
				...
				{config.postEventSummary && (
					<ChannelPicker
						...
						onChange={(channelId) => updateConfig.mutate({ summaryChannelId: channelId })}
						filter="text"
					...
					{templates.map((template) => (
						<Card key={template.id} className="p-4">
							...
							<div className="flex items-center gap-2 mt-3">
								...
								{template.recurring && <Badge variant="default">Recurring</Badge>}
							</div>
						</Card>
					))}
				</div>
...
function EventRemindersTab({ guildId }: { guildId: string }) {
	...
	const columns: Column<UpcomingReminder>[] = [
		...
		},
		{
			...
			header: "Status",
			render: (r) => (
				<Badge variant={r.sent ? "default" : "secondary"}>{r.sent ? "Sent" : "Pending"}</Badge>
			),
			width: "100px",
		},
	];
