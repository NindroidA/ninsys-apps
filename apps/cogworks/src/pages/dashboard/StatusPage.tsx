...
import { useBotStatus, useClearBotStatus, useSetBotStatus } from "@/hooks/useStatus";
import type { StatusLevel } from "@/types/status";
import { Button, Card } from "@ninsys/ui/components";
import { FadeIn } from "@ninsys/ui/components/animations";
import { AlertTriangle, CheckCircle2, Loader2, Trash2, Wrench, XCircle } from "lucide-react";
...
export function DashboardStatusPage() {
	...
	useEffect(() => {
		if (status && !hasPopulated.current) {
			...
			if (status.estimatedResolution) {
				...
				const local = new Date(status.estimatedResolution).toISOString().slice(0, 16);
	...
	const statusInfo = status ? getStatusInfo(status.level) : getStatusInfo("operational");
	const StatusIcon = statusInfo.icon;
	const isStatusSet =
		status && (status.level !== "operational" || status.message || status.isManualOverride);

	return (
		<FadeIn className="max-w-4xl">
			<PageHeader title="Bot Status" description="Manage the bot's status display (owner only)" />
			...
			<div className="space-y-6">
						...
						<div className="space-y-3">
								...
								<div>
									...
									<div className="flex flex-wrap gap-1">
										{status.affectedSystems.map((sys) => (
											<span key={sys} className="px-2 py-0.5 rounded-full bg-muted text-xs">
												{sys}
											</span>
										))}
									</div>
							...
							{status?.isManualOverride && (
								<p className="text-xs text-muted-foreground">
									...
									{status.setAt ? ` at ${new Date(status.setAt).toLocaleString()}` : ""}
								</p>
							)}
						</div>
					)}
				...
				{/* Set Status Form */}
				<ConfigSection title="Set Status" description="Update the bot's status display">
					<Select
						value={level}
					...
					<div>
						<label htmlFor={messageId} className="text-sm font-medium mb-1.5 block">
							Status Message
						</label>
						<EmojiTextarea
							id={messageId}
					...
					<div>
						<label htmlFor={resolutionId} className="text-sm font-medium mb-1.5 block">
				...
				<div className="flex items-center gap-2 flex-wrap">
					<Button onClick={() => setShowSetConfirm(true)} disabled={setStatus.isPending}>
						{setStatus.isPending ? (
							<>
