...
export function AdminAnalyticsPage() {
	...
	const totalServersConfigured = useMemo(
		() => analytics?.systemUsage?.reduce((sum, s) => sum + s.configuredCount, 0) ?? 0,
	...
	if (isLoading) {
		return (
			<FadeIn>
				<PageHeader title="Analytics" description="Bot-wide usage trends and metrics" />
			...
			<div className="space-y-6 max-w-5xl">
				...
				<div className="flex items-center justify-between flex-wrap gap-4">
					<div className="flex items-center gap-6">
						<div>
							<p className="text-2xl font-bold">{totalCommands.toLocaleString()}</p>
							<p className="text-xs text-muted-foreground">Total commands ({days}d)</p>
						</div>
						<div className="h-8 w-px bg-border" />
						<div>
							<p className="text-2xl font-bold">{totalServersConfigured}</p>
							<p className="text-xs text-muted-foreground">System configurations</p>
						</div>
					</div>
					...
					<>
						...
						<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
							...
							{(analytics.commandUsage?.length ?? 0) > 0 && (
								<Card className="p-6">
									...
									<p className="text-xs text-muted-foreground mb-4">Most used slash commands</p>
									<ResponsiveContainer width="100%" height={300}>
										<BarChart data={analytics.commandUsage.slice(0, 8)} layout="vertical">
											<CartesianGrid
												strokeDasharray="3 3"
											...
											<Bar dataKey="count" radius={[0, 6, 6, 0]} name="Uses">
												{analytics.commandUsage.slice(0, 8).map((_, i) => (
													<Cell key={`cmd-${i}`} fill={`oklch(0.65 0.16 ${220 + i * 15})`} />
												))}
											</Bar>
										</BarChart>
									</ResponsiveContainer>
							...
							{(analytics.systemUsage?.length ?? 0) > 0 && (
								<Card className="p-6">
									<h3 className="text-sm font-semibold mb-1">System Adoption</h3>
									<p className="text-xs text-muted-foreground mb-4">
									...
									<div className="space-y-3">
										{analytics.systemUsage.map((sys) => {
											...
											const pct = maxCount > 0 ? (sys.configuredCount / maxCount) * 100 : 0;
						...
						{(analytics.errorRate?.length ?? 0) > 0 && (
							<Card className="p-6">
								...
								<p className="text-xs text-muted-foreground mb-4">API and bot errors over time</p>
