...
export function BaitJoinVelocityTab({ guildId }: BaitJoinVelocityTabProps) {
  ...
  const burstActive = filtered.some(
    (e) =>
      e.isBurst && Date.now() - new Date(e.timestamp).getTime() < 5 * 60 * 1000
  );

  const currentRate = useMemo(() => {
    const fiveMinAgo = Date.now() - 5 * 60 * 1000;
    return filtered.filter((e) => new Date(e.timestamp).getTime() >= fiveMinAgo)
      .length;
  }, [filtered]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <span className="text-sm text-muted-foreground">
          Join rate:{" "}
          <span className="font-medium text-foreground">{currentRate}</span>
          /5min
        </span>
