		...
		<div className="space-y-6">
			...
			>
				...
				<StatusToggle
					...
					onChange={(pingStaffOn18Verify) => setForm((prev) => ({ ...prev, pingStaffOn18Verify }))}
							...
							<div>
								...
								{(config.workflowStatuses ?? []).length > 0 ? (
									...
									<p className="text-sm text-muted-foreground">
										No custom statuses configured. Default statuses (Open, In Progress, Awaiting
										Response, Resolved, Closed) will be used.
									</p>
								)}
							</div>
									...
									<Card className="p-3 flex items-start gap-2">
										...
										<div className="text-sm">
											...
											<span className="text-muted-foreground">
												...
												{(config.autoCloseDays ?? 7) !== 1 ? "s" : ""} inactivity
												{config.autoCloseStatus && ` in "${config.autoCloseStatus}" status`}
