...
import { Eye, Hash, Ticket as TicketIcon, UserPlus, X, XCircle } from "lucide-react";
					...
					<div className="space-y-4">
						...
						<div className="grid grid-cols-2 gap-4">
							...
							<div>
								...
								<p className="text-sm font-medium mt-0.5">{ticket.assignedTo ?? "Unassigned"}</p>
							</div>
							<div>
	...
	const columns: Column<Ticket>[] = useMemo(
		() => [
			...
			},
			{
				...
				header: "Assigned",
				render: (row) => (
					<span className={cn("text-sm", !row.assignedTo && "text-muted-foreground")}>
	...
	return (
		<div className="space-y-4">
			...
			<ConfirmDialog
				...
				title="Close Ticket"
				description={`Close ticket #${closeTarget?.id.slice(0, 8)}? This will mark it as resolved.`}
				confirmLabel="Close Ticket"
				variant="destructive"
				isLoading={closeTicket.isPending}
				onConfirm={() => {
