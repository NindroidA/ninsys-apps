	...
	return (
		...
		>
			<div className="rounded-lg border border-border bg-card p-4 space-y-4 mb-4">
				...
				<div>
					<label htmlFor={`${id}-userId`} className="text-sm font-medium mb-1.5 block">
						User ID
					</label>
					<Input
						id={`${id}-userId`}
...
export function TicketRestrictionsTab({ guildId }: TicketRestrictionsTabProps) {
	...
	const [deleteTarget, setDeleteTarget] = useState<UserTicketRestriction | null>(null);
	...
	const filtered = useMemo(() => {
		...
		return restrictions.filter((r) => r.username.toLowerCase().includes(q) || r.userId.includes(q));
	}, [restrictions, search]);
		...
		() => [
			{
				...
				render: (row) => (
					<div>
						...
						<span className="text-xs text-muted-foreground ml-2">{row.userId}</span>
					</div>
				),
			},
			{
				...
				render: (row) => (
					<span className="text-sm">
						{row.typeId ? (typeNameMap.get(row.typeId) ?? "Unknown") : "All types"}
					</span>
				),
			},
			{
	...
	return (
		<div className="space-y-4">
			...
			<AnimatePresence>
				{isAdding && <AddRestrictionForm guildId={guildId} onClose={() => setIsAdding(false)} />}
			</AnimatePresence>
			...
			<DataTable
				...
				emptyState={{
					...
					description: "No users are currently restricted from creating tickets.",
					icon: ShieldBan,
				}}
				rowActions={(row) => [
					{
