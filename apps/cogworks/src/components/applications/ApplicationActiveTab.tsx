...
function ApplicationDetailPanel({
	...
	return (
		<>
					...
					<div className="space-y-4">
						...
						{app.reviewNote && (
							<div>
								...
								<p className="text-sm mt-0.5 bg-muted/30 rounded-lg p-3">{app.reviewNote}</p>
							</div>
						)}
							...
							<div className="pt-4 border-t border-border space-y-3">
								...
								{responseEntries.map(([label, response]) => (
									<div key={label}>
										...
										<p className="text-sm mt-0.5 whitespace-pre-wrap">{response}</p>
									</div>
								))}
							</div>
						)}
			...
			>
				<div>
					<label htmlFor={approveNoteId} className="text-sm font-medium mb-1.5 block">
						Note (optional)
					</label>
					<textarea
						id={approveNoteId}
			...
			>
				<div>
					<label htmlFor={denyReasonId} className="text-sm font-medium mb-1.5 block">
						Reason (required)
					</label>
					<textarea
						id={denyReasonId}
