...
function PositionCard({
  ...
  const [editor, setEditor] = useState<PositionEditorState>(() =>
    ...
    <div className="rounded-lg border border-border overflow-visible">
      <div className="flex items-center gap-3 px-4 py-3 bg-card">
        ...
        <button
          type="button"
          aria-label={
            isExpanded ? `Collapse ${position.title}` : `Edit ${position.title}`
          }
...
export function ApplicationPositionsTab({
  guildId,
}: ApplicationPositionsTabProps) {
  ...
  const [newPosition, setNewPosition] =
    useState<PositionEditorState>(emptyEditor);
    ...
    <div className="space-y-4">
      ...
      {isAdding && (
        <div className="rounded-lg border border-primary/50 overflow-visible">
          <div className="px-4 py-3 bg-primary/5 text-sm font-medium">
            New Position
          </div>
          <PositionEditor
            state={newPosition}
            onChange={(update) =>
              setNewPosition((prev) => ({ ...prev, ...update }))
            }
            onSave={handleCreate}
            onCancel={() => {
              setIsAdding(false);
