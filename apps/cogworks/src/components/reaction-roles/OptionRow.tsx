...
import { useDeleteReactionRoleOption, useUpdateReactionRoleOption } from "@/hooks/useReactionRoles";
		...
		return (
			<div className="p-3 rounded-lg border border-primary/50 bg-primary/5 space-y-3">
				...
				<div className="flex gap-2 justify-end">
					<Button variant="ghost" onClick={handleCancel} disabled={updateOption.isPending}>
						Cancel
					</Button>
					<Button onClick={handleSave} disabled={!emoji || !roleId || updateOption.isPending}>
		...
		<>
			<div className="flex items-center gap-2 p-2 rounded-lg border border-border hover:bg-muted/30 transition-colors group">
				...
				<div className="flex-1 min-w-0">
					<span className="text-sm font-medium">{roleName ?? option.roleId}</span>
					{option.description && (
						<p className="text-xs text-muted-foreground truncate">{option.description}</p>
					)}
				</div>
				<div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
					<Button variant="ghost" onClick={() => setIsEditing(true)} className="h-7 w-7 p-0">
