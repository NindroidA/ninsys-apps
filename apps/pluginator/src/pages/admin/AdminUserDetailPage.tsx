...
import { ArrowLeft, History, Loader2, Monitor, Shield, ShieldCheck, UserX } from "lucide-react";
		...
		<div className="space-y-6">
					...
					<Card className="p-6">
						...
						<div className="grid sm:grid-cols-2 gap-4">
							...
							<div className="space-y-2">
								<label htmlFor="admin-change-tier" className="text-sm font-medium">
									Change Tier
								</label>
							...
							{/* Change Role */}
							<div className="space-y-2">
								<label htmlFor="admin-change-role" className="text-sm font-medium">
									Change Role
								</label>
								<div className="flex gap-2">
									<select
									...
									{selectedRole && (
										<Button size="sm" onClick={() => setRoleAction(selectedRole)}>
											Apply
										</Button>
									)}
								</div>
							...
							<div className="space-y-2">
								...
								<Button variant="outline" size="sm" onClick={() => setShowRevokeSessions(true)}>
									Revoke All Sessions
								</Button>
							</div>
						</div>
			...
			<FadeIn delay={0.15}>
				<Card className="p-6">
					...
					{!user.subscriptions?.length ? (
						<p className="text-sm text-muted-foreground py-2">No subscriptions.</p>
					) : (
								...
								<tbody>
									{user.subscriptions.map((sub) => (
										<tr key={sub.id} className="border-b border-border last:border-0">
											<td className="py-2 pr-4">{TIER_DISPLAY[sub.tier]?.name ?? sub.tier}</td>
			...
			<FadeIn delay={0.25}>
				<Card className="p-6">
					...
					{!user.tierHistory?.length ? (
						<p className="text-sm text-muted-foreground py-2">No tier changes.</p>
					) : (
						<div className="space-y-3">
								...
								>
									<div>
										<p className="text-sm font-medium">
											{TIER_DISPLAY[change.previousTier as keyof typeof TIER_DISPLAY]?.name ??
												change.previousTier}
											{" → "}
											{TIER_DISPLAY[change.newTier as keyof typeof TIER_DISPLAY]?.name ??
												change.newTier}
										</p>
			...
			<TwoFAVerifyDialog
				...
				description={`Change tier to ${
					tierAction?.tier
						? (TIER_DISPLAY[tierAction.tier as keyof typeof TIER_DISPLAY]?.name ?? tierAction.tier)
						: ""
			...
			<TwoFAVerifyDialog
				...
				error={deactivateUser.isError ? deactivateUser.error.message : actionError}
				onConfirm={(code) => {
					setActionError("");
			...
			<TwoFAVerifyDialog
				...
				error={revokeSessions.isError ? revokeSessions.error.message : actionError}
				onConfirm={(code) => {
					setActionError("");
