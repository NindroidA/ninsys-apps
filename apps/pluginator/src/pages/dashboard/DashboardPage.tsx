					...
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
						<div>
							...
							>
								...
								<SubscriptionBadge tier={subscription?.tier ?? user?.subscriptionTier ?? "free"} />
							</motion.div>
						</div>
						<motion.div
						...
						>
							...
							<div className="flex-1">
								<p className="text-sm font-medium">
									Your {TIER_DISPLAY[subscription.tier]?.name ?? subscription.tier} subscription
									ends on {format(new Date(subscription.currentPeriodEnd), "MMMM d, yyyy")}.
								</p>
								{subscription.downgradeToTier && (
									<p className="text-sm text-muted-foreground mt-1">
										...
										{TIER_DISPLAY[subscription.downgradeToTier]?.name ??
											subscription.downgradeToTier}{" "}
										tier. Features may be limited, but your data will be preserved.
									</p>
								)}
								<Link
									to="/account"
					...
					{[
						...
						>
							<div className="flex items-start justify-between">
								...
								<motion.div
									className={`rounded-lg p-2 ${stat.highlight ? "bg-warning/10" : "bg-primary/10"}`}
									whileHover={{ rotate: 10, scale: 1.1 }}
									transition={{ type: "spring", stiffness: 300 }}
								>
									<stat.icon
										className={`h-5 w-5 ${stat.highlight ? "text-warning" : "text-primary"}`}
									/>
								</motion.div>
							</div>
						</motion.div>
								...
								{[
									{
										...
										detail: "Get the CLI for your platform from the download page",
									},
									{
										id: "2",
										step: "2",
										action: "Configure your plugins",
										detail: "Create a pluginator.toml file or use the CLI to add plugins",
									},
									{
										id: "3",
										step: "3",
									...
									const itemProgress = Math.max(0, Math.min(1, (progress - index * 0.1) * 2));
									return (
										<motion.div
											key={step.id}
