			...
			<div className="space-y-4">
				...
				<div>
					...
					{updateProfile.isError && (
						<p className="text-sm text-red-500 mt-1">{updateProfile.error.message}</p>
					)}
				</div>
					...
					{editingBio ? (
						<div className="mt-1 space-y-2">
							...
							<div className="flex gap-2">
								<Button size="sm" onClick={handleSaveBio} disabled={updateProfile.isPending}>
									{updateProfile.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Save"}
								</Button>
...
function PasswordSection() {
	...
	const error = validationError || (changePassword.isError ? changePassword.error.message : "");
	...
	return (
		<Card className="p-6">
			...
			<form onSubmit={handleSubmit} className="space-y-4 max-w-sm">
				<div>
					<label htmlFor="current-password" className="text-sm text-muted-foreground block mb-1">
						Current Password
					</label>
				...
				<div>
					<label htmlFor="new-password" className="text-sm text-muted-foreground block mb-1">
						New Password
					</label>
					<div className="relative">
						<Input
				...
				<div>
					<label htmlFor="confirm-password" className="text-sm text-muted-foreground block mb-1">
			...
			<div className="space-y-4">
				...
				<div className="flex items-center justify-between">
					...
					{subscription?.priceFormatted && (
						<p className="text-sm text-muted-foreground">{subscription.priceFormatted}</p>
					)}
				</div>
					...
					<div className="p-3 rounded-lg bg-warning/10 border border-warning/30 text-sm">
						...
						{subscription?.downgradeToTier && (
							<p className="text-muted-foreground mt-1">
								You'll revert to{" "}
								{TIER_DISPLAY[subscription.downgradeToTier]?.name ?? subscription.downgradeToTier}{" "}
				...
				<div className="flex flex-wrap gap-2 pt-2">
					{isFree ? (
						...
						<>
							...
							>
								{portalSession.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
								Manage Billing
							</Button>
							<Link to="/pricing">
		...
		<>
			<Card className="p-6 border-red-500/20">
				...
				<p className="text-sm text-muted-foreground mb-4">
					Permanently delete your account and all associated data. This action cannot be undone. If
					you have an active subscription, it will be cancelled immediately.
				</p>
				<Button
					variant="outline"
			...
			{showDeleteDialog && (
				...
				>
					...
					<div className="relative bg-card border border-border rounded-xl shadow-2xl max-w-md w-full p-6">
						...
						<p className="text-sm text-muted-foreground mb-4">
							This will permanently delete your account, cancel any active subscriptions, and remove
							all your data. This action cannot be reversed.
						</p>
						<div className="mb-4">
							<label htmlFor="delete-confirm" className="text-sm font-medium block mb-2">
						...
						{deleteAccount.isError && (
							<p className="text-sm text-red-500 mb-4">{deleteAccount.error.message}</p>
						)}
