...
export function LoginPage() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		setFormError(result.error?.message || "Login failed. Please check your credentials.");
	};
	...
	const handle2FASubmit = useCallback(
		async (code?: string) => {
			const finalCode = code || (useRecovery ? recoveryCode.trim() : digits.join(""));
			if (!finalCode) return;

			// SEC-L1: Check challenge token expiration
			if (challengeIssuedAt && Date.now() - challengeIssuedAt > CHALLENGE_TTL_MS) {
			...
			if (result.success) {
				...
				if (newAttempts >= MAX_2FA_ATTEMPTS) {
					...
					setTwoFAError("Too many failed attempts. Please wait 30 seconds before trying again.");
				} else {
					...
					>
						{useRecovery ? (
							<div className="space-y-4">
								...
								/>
								{twoFAError && <p className="text-sm text-error text-center">{twoFAError}</p>}
								<Button
									variant="primary"
									className="w-full"
