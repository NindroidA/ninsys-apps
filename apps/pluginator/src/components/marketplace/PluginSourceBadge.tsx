...
import { SiCurseforge, SiGithub, SiJenkins, SiModrinth, SiSpigotmc } from "react-icons/si";
...
function SourceIcon({ type, className }: { type: string; className?: string }) {
	...
	switch (type) {
		case "modrinth":
			return <SiModrinth {...iconProps} style={{ color: SOURCE_COLORS.modrinth }} />;
		case "github":
			return <SiGithub {...iconProps} />;
		case "spigot":
			return <SiSpigotmc {...iconProps} style={{ color: SOURCE_COLORS.spigot }} />;
		case "hangar":
			// Hangar uses a paper plane style icon (PaperMC's platform)
			return <Send {...iconProps} style={{ color: SOURCE_COLORS.hangar }} />;
		case "curseforge":
			return <SiCurseforge {...iconProps} style={{ color: SOURCE_COLORS.curseforge }} />;
		case "jenkins":
			return <SiJenkins {...iconProps} style={{ color: SOURCE_COLORS.jenkins }} />;
		default:
...
export function PluginSourceBadge({
	...
	const content = (
		<>
			...
			{asLink && url && <ExternalLink className={cn(iconSizes[size], "opacity-50")} />}
		</>
	);
	...
	return (
		<div className="flex items-center gap-1.5">
			{visibleSources.map((source) => (
				<span
					key={`${source.type}-${source.projectSlug || source.resourceId || source.repoSlug}`}
					...
					title={SOURCE_INFO[source.type].name}
				>
