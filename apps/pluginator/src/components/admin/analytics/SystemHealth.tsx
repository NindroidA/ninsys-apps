...
export function SystemHealth() {
	...
	const totalSessionAuth = data.sessionsByAuth.reduce(
		(sum, s) => sum + s.count,
		0,
	);
	const twoFaPct =
		data.twoFaTotal > 0
			? Math.round((data.twoFaEnabled / data.twoFaTotal) * 100)
			: 0;
	const twoFaAdminPct =
	...
	const tokenPct =
		data.totalUsers > 0
			? Math.round((data.usersWithTokens / data.totalUsers) * 100)
			: 0;
							...
							<div key={s.method} className="space-y-1">
								<div className="flex justify-between text-xs">
									...
									<span className="text-muted-foreground">
										{s.count} (
										{totalSessionAuth > 0
											? Math.round((s.count / totalSessionAuth) * 100)
											: 0}
										%)
									</span>
								</div>
				...
				<div className="space-y-4">
					...
					<div className="space-y-1">
						...
						<ProgressBar
							...
							className={
								twoFaAdminPct === 100 ? "bg-emerald-500" : "bg-amber-500"
							}
						/>
						<p className="text-xs text-muted-foreground">
					...
					<StatRow
						...
						value={(
							data.totalUsers > 0 ? data.totalTokens / data.totalUsers : 0
						).toFixed(2)}
					/>
				</div>
			</div>
