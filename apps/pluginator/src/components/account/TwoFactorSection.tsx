...
export function TwoFactorSection() {
	...
	return (
		<>
						...
						{showDisable ? (
							<div className="space-y-3 max-w-sm">
								...
								<Input
									...
									autoFocus
									onKeyDown={(e) => {
										if (e.key === "Enter" && disableCode.length === 6) handleDisable();
										if (e.key === "Escape") {
											setShowDisable(false);
											setDisableCode("");
										}
								...
								/>
								{disable2FA.isError && (
									<p className="text-sm text-red-500">{disable2FA.error.message}</p>
								)}
			...
			<TwoFactorSetupDialog open={showSetup} onClose={() => setShowSetup(false)} />
		</>
	);
}
