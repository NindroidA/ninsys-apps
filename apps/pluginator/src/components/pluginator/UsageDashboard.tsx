...
import { TIER_DISPLAY, type UsageStat, getTierLevel } from "@/types/tier";
...
export function UsageDashboard() {
	...
	const isCanceling = status === "canceling" || subscription?.cancelAtPeriodEnd === true;
		...
		<div className="space-y-6">
			...
			<Card className="p-6">
				...
				<div className="mt-4 pt-4 border-t border-border">
					...
					{currentLevel === 0 && hasPlusDiscount && (
						<div className="space-y-3">
							<p className="text-sm text-muted-foreground">
								You're on the Free plan with Plus discounts available. Subscribe to Pro or Max at a
								reduced rate.
							</p>
							<Button size="sm" onClick={() => navigate("/pricing")}>
								View Plans
					...
					{effectiveTier === "plus" && !hasSubscription && (
						<div className="space-y-3">
							<p className="text-sm text-muted-foreground">
								Your Plus purchase is active. Subscribe to Pro or Max for even more power.
							</p>
							<Button size="sm" onClick={() => navigate("/pricing")}>
								View Plans
							</Button>
								...
								<div className="flex items-start gap-2">
									...
									<div className="text-sm">
										...
										<p className="text-muted-foreground">
											Please update your payment method to keep your {tierInfo?.name} plan active.
										</p>
									</div>
								</div>
							</div>
						...
						<div className="space-y-3">
							<div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
								<div className="flex items-start gap-2">
									...
									<div className="text-sm">
										<p className="font-medium text-amber-500">Subscription ending</p>
										<p className="text-muted-foreground">
											Your {tierInfo?.name} plan will remain active until{" "}
											<span className="font-medium text-foreground">{periodEndDate}</span>. After
											that, you'll be on the {fallbackTier} plan.
										</p>
									</div>
								</div>
							</div>
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				...
				<UsageCard
					...
					stat={usage.today.migrations}
				/>
			</div>

			{/* Low Usage Warning */}
			{(isLowUsage(usage.today.updateChecks) ||
				isLowUsage(usage.today.downloads) ||
				isLowUsage(usage.today.syncs) ||
				isLowUsage(usage.today.migrations)) && (
			...
			<ConfirmDialog
				...
				description={`Your ${tierInfo?.name ?? "current"} plan will remain active until ${
